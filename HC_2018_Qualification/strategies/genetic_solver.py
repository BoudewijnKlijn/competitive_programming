...
class GeneticStrategy(Strategy):
    ...
    def __init__(self, scorer, seed=1, max_generations=1000, population_size=2, crossover_rate=0.5,
        ...
        Args:
            scorer (valcon.Scorer): Scorer to calculate score
            ...
            crossover_rate (float): Cross over rate (i.e. how much of the content to use from parent1 and parent2)
            mutation_rate (float: Mutation rate (i.e. the probability of mutations)
    ...
    def _generate_initial_population(self, input_data):
        """
        Generates an initial population
        """
        return [RandomSolver().solve(input_data) for _ in range(0, self.population_size)]

    def _select_parents(self, population, scores: [int]):
        """
    ...
    def _cross_over(self, parent1, parent2):
        ...
        """
        raise NotImplementedError("How to combine car schedules??")
        # children are copies of parents by default
        children1, children2 = parent1, parent2
        # check for recombination
        if np.random.rand() < self.crossover_rate:
            # select crossover point that is not on the end of the items
            pt = np.random.randint(1, len(parent1.car_schedules))
            # perform crossover
            # todo: use set characteristics to improve the below two lines
            rides1 = [schedule.ride for schedule in parent1.car_schedules]
            rides2 = [schedule.ride for schedule in parent1.car_schedules]

            children1_rides = set(list(parent1.car_schedules)[:pt] + list(parent2.car_schedules)[pt:])
            children1 = CarSchedules(children1_rides)
            children2_rides = set(list(parent2.car_schedules)[:pt] + list(parent1.car_schedules)[pt:])
            children2 = CarSchedules(children2_rides)
    ...
    def _mutation(self, child: [str]):
        ...
        """
        raise NotImplementedError("How to mutate car schedules??"
        for i in range(len(child.car_schedules) - 1):
            current_item = list(child.car_schedules)[i]
            next_item = list(child.car_schedules)[i + 1]
            # check for a mutation
            if np.random.rand() < self.mutation_rate:
                # switch the items
                list(child.car_schedules)[i] = next_item
                list(child.car_schedules)[i + 1] = current_item
    ...
    def solve(self, input_data):
        ...
        for i in tqdm(range(0, self.max_generations)):
            ...
            if new_max_score > best_solution_score:
                ...
                best_solution, best_solution_score = deepcopy(population[idx_new_max_score]), scores[
                    idx_new_max_score]
